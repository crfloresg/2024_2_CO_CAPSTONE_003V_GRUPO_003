...
export class DashboardComponent {
    ...
    try {
        if (this.informeSelected !== undefined) {
            ...
            if (this.informeSelected === 0) {
              ...
              req = await this.informesService.auditoria(this.informeUsuarioId);
            }
  ...
  async getDashboard() {
    ...
    try {
      this.perdidas = await this.dashboardService.getPerdidas(this.dashboardBodegaId);
      this.solicitudes = await this.dashboardService.getSolicitados(this.dashboardBodegaId);
      this.Transferencia = await this.dashboardService.getTransferidos(this.dashboardBodegaId);
    } catch (error) {
      this.uiService.showErrorModal('Error al cargar datos', error);
    } finally {
