...
export class AuthService {
  ...
  getBodega(){
    return localStorage.getItem(BODEGA_KEY) || '';
  }
  ...
  async getBodegaUsuario(){
    try {
      const req = await lastValueFrom(this.httpClient.get<Bodega>(`${this.apiUrl}/GetBodega`));
      localStorage.setItem(BODEGA_KEY, req.nombre);
      return req;
    } catch (error) {
      throw error;
    }
