...
namespace InformeApi.Controllers
{
    ...
    public class InformeController : Controller
    {
        ...
        [HttpGet("Auditoria")]
        public async Task<IActionResult> PdfAuditoria(int usuId,string startDateStr,string endDateStr)
        {
            ...
            var informeAuditor = await GetAuditorias(usuId, bodegaId, startDateStr, endDateStr);
            var reporte = GeneeratePdfService.GetAuditoria(informeAuditor, nameEmployee, nameBodega, bodegaId, nametipoBodega, startDateStr, endDateStr);
            byte[] pdfBytes = reporte.GeneratePdf();
            var mimeType = "application/pdf";
        ...
        public async Task<IActionResult> PdfSolicitud(int solicitudId)
        {
            var userRequest = _tokenProvider.GetAccessTokenData();

            if (!_tokenProvider.HasPermission("r_compras_global")) { return Forbid(); }

            var infoSolicitud = await SolicitudProuc(solicitudId);

            var reporte = GeneeratePdfService.GetSolicitudDetalle(infoSolicitud);
            byte[] pdfBytes = reporte.GeneratePdf();
            var mimeType = "application/pdf";
            var fileName = $"Informe compra productos {fechas}.pdf";
        ...
        private async Task<List<DataAuditor>> GetAuditorias(int usuId, int bodegaId, string startDateStr, string endDateStr)
        {
            ...
            try
            {
                var query = from a in _context.Auditorias
                            ...
                            where (bodegaId == 1 || b.BodegaId == bodegaId)
                                && u.UsuarioId == usuId
                                && a.FechaAccion.Date >= startDate.Date && a.FechaAccion.Date <= endDate.Date
        ...
        private async Task<List<Solicitud>> SolicitudProuc(int solicitudId)
        {
            try
            {
                var userRequest = _tokenProvider.GetAccessTokenData();

                var query = from s in _context.SolicitudInventarios
                            join ds in _context.SolicitudInventarioDetalles on s.SolicitudId equals ds.SolicitudId
                            join p in _context.Productos on ds.ProductoId equals p.ProductoId
                            where s.SolicitudId == solicitudId
                            select new Solicitud
                            {
                                Id = s.SolicitudId,
                                fechasSolicitud = s.FechaSolicitud.ToString("dd/MM/yyyy"),
