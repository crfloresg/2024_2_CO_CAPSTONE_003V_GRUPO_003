...
namespace InformeApi.Services
{
    ...
    public class GeneeratePdfService() : GeneratePdf
    {
        ...
        public static Document GetAuditoria(List<DataAuditor> informeAuditor, string nameEmployee, string nameBodega, int bodegaid, string nametipoBodega, string startDateStr, string endDateStr)
        {
            var report = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Header().Row(row =>
                    {
                        ...
                        row.RelativeItem(60)
                            .AlignCenter()
                            .PaddingTop(20)
                            .Text("Informe de Historial de actividad de usuarios")
                            .SemiBold().FontSize(18).FontColor(Colors.Black);
                        row.RelativeItem(30)
                            .AlignRight()
                            .Text($"Fecha : {DateTime.Now:dd/MM/yyyy}")
                    ...
                    page.Content().Padding(25).Column(column =>
                    {
                        column.Item().Text($"Emisor : {nameEmployee}");
                        column.Item().Text($"Bodega : {nameBodega}");
                        column.Item().Text($"Fecha inicio : {startDateStr}");
                        column.Item().Text($"Fecha termino : {endDateStr}");
                        column.Item().Height(10);
        ...
        public static Document GetSolicitudDetalle(List<Solicitud> infoSolicitud)
        {
            var report = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Content().Padding(25).Column(column =>
                    {
                        ...
                        var groupedItems = infoSolicitud.GroupBy(item => item.Id);
                        foreach (var group in groupedItems)
                        {
                            var firstItem = group.First();
                            column.Item().Height(5);
                            column.Item().Text($"Fecha solicitud : {firstItem.fechasSolicitud}");
                            column.Item().Text($"Observación : {firstItem.observacion}");
                        }
                        column.Item().Height(15);
                        ...
                        column.Item().Table(table =>
                        {
                            ...
                            table.Header(header =>
                            {
                                ...
                                header.Cell().BorderTop(1).BorderBottom(1).AlignCenter().Padding(3).Text("Nombre");
                                header.Cell().BorderTop(1).BorderBottom(1).AlignCenter().Padding(3).Text("Codigo");
                                header.Cell().BorderTop(1).BorderBottom(1).AlignCenter().Padding(3).Text("Categoria");
                                header.Cell().BorderTop(1).BorderBottom(1).AlignCenter().Padding(3).Text("Unidad de medida");
                                header.Cell().BorderTop(1).BorderBottom(1).AlignCenter().Padding(3).Text("Cantidad en bodega");
                                header.Cell().BorderTop(1).BorderBottom(1).AlignCenter().Padding(3).Text("Cantidad aprobada");
                            });

                            table.Cell().ColumnSpan(7).Height(5);

                            int rowIndex = 0;
                            int numero = 1;
                            foreach (var sl in infoSolicitud)
                            {
                                var backgroundColor = rowIndex % 2 == 0 ? Colors.White : Colors.Grey.Lighten2;

                                table.Cell().Background(backgroundColor).Padding(3).AlignCenter().PaddingLeft(5).Text($"{numero}");
                                table.Cell().Background(backgroundColor).Padding(3).PaddingLeft(5).Text($"{sl.producto}");
                                table.Cell().Background(backgroundColor).Padding(3).PaddingLeft(5).Text($"{sl.codigo}");
                                table.Cell().Background(backgroundColor).Padding(3).PaddingLeft(5).AlignRight().Text($"{sl.categoria}");
                                table.Cell().Background(backgroundColor).Padding(3).PaddingLeft(5).AlignRight().Text($"{sl.unidad}");
                                table.Cell().Background(backgroundColor).Padding(3).PaddingLeft(5).AlignRight().Text($"{sl.valor}"); //// agregar cantidad en bodega central
                                table.Cell().Background(backgroundColor).Padding(3).PaddingLeft(5).AlignRight().Text($"{sl.cantidad}");
