  ...
  public class TransferenciasController : ControllerBase
  {
    ...
    public async Task<IActionResult> CreateTrasnferencia([FromBody] TransferenciaCreateRequest body)
    {
      ...
      try
      {
        ...
        var sol = await _context.SolicitudInventarios.Where(x => x.SolicitudId == body.solicitudId).FirstOrDefaultAsync();
        sol.EstadoSolicitudId = 4;
        await _context.SaveChangesAsync();
    ...
    //Entrega los detalles de la solicitud para rellenar data en transferencia
    [HttpGet("SolicitudDetails/{idSolicitud}")]
    public async Task<IActionResult> SolicitudDetails(int idSolicitud)
    {
      try
      {
        var userRequest = _tokenProvider.GetAccessTokenData();
        ...
        var sol = await _context.SolicitudInventarios.Where(x => x.SolicitudId == idSolicitud && x.EstadoSolicitudId == 2).FirstOrDefaultAsync();

        if(sol == null) { return NotFound(); }

        return Ok(sol.BodegaId);
